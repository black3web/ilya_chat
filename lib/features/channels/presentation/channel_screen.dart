// File: lib/features/channels/presentation/channel_screen.dart
...
class _ChannelScreenState extends ConsumerState<ChannelScreen> {
  ...
  Future<void> _send() async {
    ...
    if (user == null) return;
    _msgCtrl.clear();
    await ref.read(firestoreServiceProvider).sendChannelMessage(
  ...
  Widget build(BuildContext context) {
    ...
    final channelAsync = ref.watch(userChannelsProvider(user.id));
    final canPost = channelAsync.when(
      data: (channels) {
        final ch =
            channels.where((c) => c.id == widget.channelId).firstOrNull;
        if (ch == null) return false;
        return ch.ownerId == user.id || ch.adminIds.contains(user.id);
      },
      loading: () => false,
      error: (_, __) => false,
    );

    final msgsAsync = ref.watch(channelMessagesProvider(widget.channelId));
